...
| R-02 | `04_api_spec.yaml`, `05_screen_spec.md`, `07_authz_authn.md` | API エンドポイント一覧が現実装と不一致 | Partial | `04_api_spec.yaml` は更新済み。残件は周辺資料更新と自動生成との差分運用整理 | In Progress | TODO は `api_spec_todo.md` で管理。`07_authz_authn.md` 追随は未完了 |
...
| R-07 | `04_api_spec.yaml`, `07_authz_authn.md` | エラーレスポンスが独自 `ErrorResponse` 前提のまま | Partial | `04_api_spec.yaml` は `ProblemDetail` ベースへ更新済み。`07_authz_authn.md` を追随させる | In Progress | `GlobalExceptionHandler` 準拠 |
