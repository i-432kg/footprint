...
- 対象クラス: `RootController`, `LocalFrontendAssetModelAdvice`, `BuiltFrontendAssetModelAdvice`, `ViteManifestAssetResolver`
...
| 5 | 異常系 | manifest 不在、読み込み失敗、必須エントリ欠落で `IllegalStateException` を送出すること |
...
- `RootController` と advice はクラス単体 UT で十分
