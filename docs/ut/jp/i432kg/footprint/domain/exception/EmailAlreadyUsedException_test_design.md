...
| 2 | 正常系 | details | `emailAddress` を details に保持すること |
...
|---|---|---|---|---|---|
| 1 | 正常系 | 例外情報を保持する | `EmailAddress.of("user@example.com")` | `EMAIL_ALREADY_USED`, 固定 message, `emailAddress` details |  |
...
- 備考:
  - `details` は `target` / `reason` 形式ではなく `emailAddress` のみを持つ

## 6. 対応するテストメソッド

| No. | テストメソッド名 | `@DisplayName` |
|---|---|---|
| 1 | `should_setMessageErrorCodeAndDetails_when_constructed` | `EmailAlreadyUsedException は message と errorCode と emailAddress details を保持する` |
